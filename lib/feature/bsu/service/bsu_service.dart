...
class BSUService {
  ...
  Future<Either<Failure, BaseResponse<DetailTransactionBsuModel>?>>
      getDetailTransaction(String idTransaction) async {
    try {
      final response = await http
          .get(Uri.parse("$getDetailTransactionBSUUrl$idTransaction"));
      var res = json.decode(response.body);
      if (response.statusCode == 200) {
        if (res["status"] == "true") {
          final result = BaseResponse<DetailTransactionBsuModel>.fromJson(
              res, (data) => DetailTransactionBsuModel.fromJson(data));
          return Right(result);
        } else {
          return Left(ServerFailure(res["result"].toString()));
        }
